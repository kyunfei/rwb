...
export '../utils/book_metadata_merge.dart';

/// 书籍数据提供者抽象接口
/// 预留在线书籍接口，本地和在线书籍统一抽象
abstract class BookDataProvider {
  /// 获取书籍信息
  /// [fallbackBook]：搜索/书架已有元数据；详情规则取不到字段时回退使用。
  Future<Book?> getBookInfo(String bookUrl, {Book? fallbackBook});
...
class OnlineBookDataProvider implements BookDataProvider {
  ...
  Future<Book?> getBookInfo(String bookUrl, {Book? fallbackBook}) async {
    ...
    return webBook.getBookInfo(bookUrl, fallbackBook: fallbackBook);
  }
